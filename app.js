...
export const floorData={
 ...
 '1f':{name:'1층',image:'assets/전시동 1층.png',locations:[['hall1','제1전시장'],['hall2','제2전시장'],['hall3','제3전시장'],['multi','다목적홀'],['auditorium','강당']],exits:[['서쪽 비상구',8.1,49.2],['남서쪽 비상구',50.6,93.3],['남쪽 비상구',74,92.6],['동북쪽 비상구',91.7,41.1],['동쪽 비상구',92.6,67.9],['동남쪽 비상구',90.7,77.9]]},
...
export const navigation={
 b1:{rooms:{
  ...
  'east-elevator':[[85,41,90,49],[85,50],'e','동쪽 비상구',[[[88,49.5],'e']]],'west-parking':[[5,45,18,75],[18,65],'wWest','서쪽 비상구',[[[16,60],'w']],'open-zone'],'central-parking':[[25,66,55,82],[45,65],'c2','중앙 비상구',[[[30,68],'w'],[[47,78],'south'],[[54,68],'c3']],'open-zone'],'south-parking':[[30,83,65,94],[47,78],'south','남측 중앙 비상구',[[[35,82],'w'],[[62,82],'c3']],'open-zone'],'east-ramp':[[84,35,92,76],[84,58],'e','동쪽 비상구',[[[84,38],'eNorth'],[[91,52],'east']],'open-zone']},
 ...
 '2f':{rooms:{
  ...
  '213':[[57,87.5,63,92],[57,89.5],'l4','중앙 비상구 2'],'214':[[72,75,80,87],[72,79],'r214','동남쪽 비상구']},
  nodes:{nw:[12.7,40.7],west:[8.7,65.7],u0:[10,52],u1:[18,52],u2:[23,52],u3:[31,52],u4:[39,52],u5:[43.5,52],u6:[50,52],u7:[56,52],u8:[72,52],hallExit:[76.8,50.4],eastTurn:[83,50.4],eastNorth:[87,40],ne:[89.6,32.1],east:[95.5,46.4],d201:[16,65.8],d202:[23,65.8],d203:[30.5,65.8],c1:[36.8,65.7],d204:[38,65.8],d205:[43.5,65.8],c2:[52.1,65.9],d206:[50,65.9],d207:[56,65.9],lowerTop:[69.8,65.9],l1:[55,70],l2:[55,77],l3:[55,84],l4:[55,89.5],r1:[70,70],r2:[70,77],r214:[70,79],r3:[70,84],r4:[70,89.5],top214:[76,72],eastUpper:[82,72],eastSide:[82,82],south214:[75,90],eastLower:[82,89.5],se:[81.4,81.5]},
  ...
  // corridor through the turquoise boundary. Room 214 has no east-side door:
  // its southeast route leaves by the west door and follows the outside of its
  // south and east walls before reaching the exit.
...
export function eligibleRouteExits(exits,excludeExits=[]){const excluded=new Set(excludeExits);return exits.filter(exit=>!excluded.has(exit[0]))}
function planRoute({startPoint=null,blockedSegment=null,blockedSegments=[],excludeExit=null,excludeExits=[]}={}){
 ...
 const excludedSegments=[...blockedSegments,...(blockedSegment?[blockedSegment]:[])],samePoint=(a,b)=>Math.hypot(a[0]-b[0],a[1]-b[1])<.2,sameSegment=(a,b)=>excludedSegments.some(segment=>(samePoint(nav.nodes[a],segment[0])&&samePoint(nav.nodes[b],segment[1]))||(samePoint(nav.nodes[a],segment[1])&&samePoint(nav.nodes[b],segment[0]))),nodes={...nav.nodes,start},links=nav.links.filter(([a,b])=>!sameSegment(a,b));portals.forEach(([door,entry],index)=>{const name=`door${index}`;nodes[name]=door;links.push(['start',name],[name,entry])});
 ...
 const eligibleExits=eligibleRouteExits(floorData[floor].exits,[...excludeExits,...(excludeExit?[excludeExit]:[])]),candidates=eligibleExits.map(exit=>{const names=strict[nav.exitNodes[exit[0]]];return names&&{exit,path:names.map(name=>nodes[name]),blocked:false}}).filter(Boolean);
 if(!candidates.length){const nearest=[...eligibleExits].sort((a,b)=>Math.min(...portals.map(([door])=>Math.hypot(a[1]-door[0],a[2]-door[1])))-Math.min(...portals.map(([door])=>Math.hypot(b[1]-door[0],b[2]-door[1]))))[0]||['사용 가능한 비상구 없음',start[0],start[1]];return{exit:nearest,path:[start,portals[0]?.[0]||start],blocked:true,noPath:true}}
...
let cameraStream=null,torchOn=false,sensorActive=false,voiceOn=true,signalOn=false,signalTimer=null,signalVoiceTimer=null,audioContext=null,remainingDistance=24,lastStepAt=0,lastMagnitude=0,targetBearing=0,currentHeading=null,lastDirection='',lastSpokenAt=0,lastDistanceCallout=Infinity,lastWrongWayAt=0,arSteps=[],arStepIndex=0,arCompleted=false,arDestinationName='비상구',arRoute=null,rejectedExits=new Set(),rejectedSegments=[];
let obstacleModel=null,obstacleTimer=null,obstacleRetryTimer=null,obstacleBusy=false,obstacleBlocked=false,obstaclePromptShown=false,obstacleSince=0,lastDetourAt=0,sceneBlockedFrames=0,sceneCanvas=null,orientationReceived=false,motionReceived=false,sensorCheckTimer=null;
...
export function mobilityMotionProfile(status=''){
 if(status.includes('휠체어'))return{distance:.3,minDelta:.65,maxDelta:4.5,cooldown:520,label:'휠체어 이동 감지'};
 if(status.includes('보행 보조기구'))return{distance:.4,minDelta:1.15,maxDelta:6,cooldown:600,label:'보행 보조 이동 감지'};
 if(status.includes('어려워')||status.includes('도움'))return{distance:.25,minDelta:.9,maxDelta:5,cooldown:750,label:'느린 보행 감지'};
 return{distance:.7,minDelta:1.8,maxDelta:7,cooldown:380,label:'일반 보행 감지'};
}
...
async function ensureTrackingSession(route){trackingId||=globalThis.crypto?.randomUUID?.().replaceAll('-','')||`${Date.now()}${Math.random().toString(36).slice(2)}`;await postTracking(route);clearInterval(trackingTimer);trackingTimer=setInterval(()=>postTracking(arRoute||route),15000);return`${location.origin}/guardian.html?track=${encodeURIComponent(trackingId)}`}
...
function renderDetourHistory(){const box=$('#detourHistory');if(!box)return;box.textContent=rejectedExits.size?`제외된 출구: ${[...rejectedExits].join(', ')}`:'제외된 경로 없음';$('#resetRouteButton').disabled=!rejectedExits.size&&!rejectedSegments.length}
function renderArMap(){if(!arRoute)return;const data=floorData[floor],points=arRoute.path.map(point=>point.join(',')).join(' ');$('#arMapImage').src=data.image;$('#arMapImage').alt=`${data.name} 대피 경로 지도`;$('#arMapTitle').textContent=`${data.name} ${currentLocation()[1]} → ${arDestinationName}`;$('#arMapRoute').innerHTML=`<polyline class="ar-map-route-shadow" points="${points}"/><polyline class="ar-map-route-line" points="${points}"/><circle class="ar-map-exit" cx="${arRoute.exit[1]}" cy="${arRoute.exit[2]}" r="1.3"/>`;renderDetourHistory();updateRouteProgress()}
...
function recalculateDetour(source='신고된 통로'){if(!arRoute)return;const previousExit=arDestinationName,progress=routeProgress(arSteps,arStepIndex,remainingDistance),point=pointAlongPath(arRoute.path,progress.percent),blockedSegment=segmentAtProgress(arRoute.path,progress.percent);rejectedExits.add(previousExit);if(blockedSegment)rejectedSegments.push(blockedSegment);const next=planRoute({startPoint:point,blockedSegments:rejectedSegments,excludeExits:[...rejectedExits]});renderDetourHistory();if(next.noPath){$('#arMessage').textContent='확인한 막힌 경로를 모두 제외하면 남은 안전 경로가 없습니다. 필요하면 우회 기록을 초기화하세요.';speakGuide('남은 안전한 우회로가 없습니다. 안전한 곳에 멈추거나 우회 기록을 초기화하세요.',true);return}arRoute=next;arDestinationName=next.exit[0];arSteps=buildArSteps(next);arStepIndex=0;arCompleted=false;obstacleBlocked=false;renderArMap();activateArStep(0,false);$('#arMessage').textContent=`${previousExit} 경로를 제외했습니다. ${arDestinationName}로 안내합니다.`;speakGuide(`${source}를 피해 ${arDestinationName}로 우회 안내합니다. ${arSteps[0].text}`,true)}
function resetDetourHistory(){if(!arRoute)return;const progress=routeProgress(arSteps,arStepIndex,remainingDistance),point=pointAlongPath(arRoute.path,progress.percent);rejectedExits.clear();rejectedSegments=[];const next=planRoute({startPoint:point});if(next.noPath)return;arRoute=next;arDestinationName=next.exit[0];arSteps=buildArSteps(next);arStepIndex=0;arCompleted=false;renderArMap();activateArStep(0,false);$('#arMessage').textContent=`우회 기록을 초기화했습니다. 현재 위치에서 ${arDestinationName}로 다시 안내합니다.`;speakGuide(`우회 기록을 초기화했습니다. ${arDestinationName}로 다시 안내합니다.`,true)}
async function startAr(){
 rejectedExits.clear();rejectedSegments=[];const route=planRoute(),item=currentLocation();arRoute=route;arDestinationName=route.exit[0];arSteps=buildArSteps(route);arCompleted=false;currentHeading=null;lastWrongWayAt=0;lastTrackingMoveAt=Date.now();$('#arLocation').textContent=`${floorData[floor].name} ${item[1]}`;show(5);renderArMap();activateArStep(0,false);setArrowMode('forward');
...
function setObstacleState(blocking){
 ...
 if(!obstacleBlocked){obstacleSince=0;obstaclePromptShown=false;setObstacleUi('active','AI 장애물 인식 켜짐');return}const labels=[...new Set(blocking.map(item=>item.class))].join(', ');status.textContent=`전방 대형 물체 감지 · ${labels}`;obstacleSince||=Date.now();if(!obstaclePromptShown){obstaclePromptShown=true;$('#arMessage').textContent='전방 장애물이 감지되었습니다. 통과하기 어렵다면 경로 지도에서 우회하세요.';speakGuide('전방 장애물이 감지되었습니다. 통과하기 어렵다면 경로 지도에서 우회하세요.',true)}
}
...
if(typeof document!=='undefined'){$('#close').onclick=$('#modalConfirm').onclick=()=>$('#modal').classList.remove('open');$('#arStart').onclick=startAr;$('#homeArStart').onclick=startAr;$('#kakaoLocation').onclick=shareKakaoLocation;$('#arClose').onclick=()=>show(1);$('#mapButton').onclick=()=>toggleArMap();$('#mapClose').onclick=()=>toggleArMap(false);$('#blockedRouteButton').onclick=()=>recalculateDetour('신고된 막힌 통로');$('#resetRouteButton').onclick=resetDetourHistory;$('#arMapModal').onclick=event=>{if(event.target===$('#arMapModal'))toggleArMap(false)};$('#torchButton').onclick=toggleTorch;$('#voiceButton').onclick=toggleVoice;$('#signalButton').onclick=toggleSignal;$('#guardian').onclick=()=>modal('보호자에게 알리기',alertSummary('보호자'));$('#manager').onclick=()=>modal('관리자에게 알리기',alertSummary('관리자'));window.addEventListener('keydown',event=>{if(event.key==='Escape'&&!$('#arMapModal').hidden)toggleArMap(false)});window.addEventListener('pagehide',stopAr);renderFloor()}
