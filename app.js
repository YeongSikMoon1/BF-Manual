...
let obstacleModel=null,obstacleTimer=null,obstacleBusy=false,obstacleBlocked=false,lastObstacleWarningAt=0;
...
export function arrowMode(text=''){return text.includes('왼쪽')?'left':text.includes('오른쪽')?'right':'forward'}
...
function updateRouteProgress(){const progress=routeProgress(arSteps,arStepIndex,remainingDistance);$('#routeTrackFill').style.width=`${progress.percent}%`;$('#routeWalker').style.left=`${progress.percent}%`;$('#routeProgressLabel').textContent=arCompleted?'안전지대 도착':`${progress.percent}% 이동 · 구간 ${arStepIndex+1}/${arSteps.length}`;$('#routeRemaining').textContent=arCompleted?'안내 유지 중':`전체 약 ${Math.ceil(progress.remaining)}m 남음`}
function setArrowMode(){const arrow=$('#arArrow'),mode=arrowMode(arSteps[arStepIndex]?.text);arrow.classList.remove('arrow-forward','arrow-left','arrow-right');arrow.classList.add(`arrow-${mode}`);arrow.dataset.mode=mode}
...
function updateDirection(rotation){const normalized=((rotation+540)%360)-180,arrow=$('#arArrow'),mode=arrow.dataset.mode||'forward';arrow.style.setProperty('--heading-rotation',`${normalized}deg`);arrow.style.setProperty('--turn-rotation',mode==='left'?'-90deg':mode==='right'?'90deg':'0deg')}
...
async function loadObstacleModel(){
 ...
 $('#obstacleStatus').textContent='대형 물체 인식 모델 불러오는 중';
...
function setObstacleState(blocking){
 ...
 if(!obstacleBlocked){status.textContent='전방 대형 물체 인식 중';return}const labels=[...new Set(blocking.map(item=>item.class))].join(', ');status.textContent=`전방 대형 물체 감지 · ${labels}`;$('#arMessage').textContent='멈추세요. 전방 장애물을 피해 안전을 확인하세요.';const now=Date.now();if(now-lastObstacleWarningAt>5000){lastObstacleWarningAt=now;speakGuide('멈추세요. 전방에 장애물이 감지되었습니다.',true)}
}
async function startObstacleDetection(){
 stopObstacleDetection();try{await loadObstacleModel();if(!cameraStream)return;$('#obstacleStatus').textContent='전방 대형 물체 인식 중';obstacleTimer=setInterval(detectObstacles,850);detectObstacles()}catch(error){console.error('Obstacle detection failed',error);$('#obstacleStatus').textContent='물체 인식 재시도 필요';$('#arMessage').textContent=`물체 인식 모델을 불러오지 못했습니다. 네트워크를 확인하고 AR을 다시 시작하세요. (${error.message})`}
}
