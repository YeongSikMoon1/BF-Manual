...
let cameraStream=null,torchOn=false,sensorActive=false,voiceOn=true,signalOn=false,signalTimer=null,audioContext=null,remainingDistance=24,lastStepAt=0,lastMagnitude=0,targetBearing=0,lastDirection='',lastSpokenAt=0,lastDistanceCallout=Infinity;
function arRouteBearing(){const route=planRoute(),a=route.path[0],b=route.path[1]||route.path[0];return(Math.atan2(b[0]-a[0],a[1]-b[1])*180/Math.PI+360)%360}
function updateDirection(rotation){const normalized=((rotation+540)%360)-180,label=Math.abs(normalized)<22?'앞쪽으로 이동하세요':normalized>0?'오른쪽으로 이동하세요':'왼쪽으로 이동하세요';$('#arArrow').style.transform=`rotateZ(${normalized}deg) rotateX(52deg)`;$('#arDirection').textContent=label;if(label!==lastDirection&&Date.now()-lastSpokenAt>4000){lastDirection=label;speakGuide(label)}}
async function startAr(){
 const route=planRoute(),item=currentLocation();targetBearing=arRouteBearing();remainingDistance=Math.max(8,Math.round(pathLength(route.path)*1.45));lastDistanceCallout=remainingDistance;
 $('#arDistance').textContent=remainingDistance;$('#arExitName').textContent=`${route.exit[0]} 방향`;$('#arLocation').textContent=`${floorData[floor].name} ${item[1]}`;show(5);updateDirection(targetBearing);
 if(!navigator.mediaDevices?.getUserMedia){$('#arMessage').textContent='이 브라우저에서는 카메라를 사용할 수 없습니다.';return}
 try{const result=await connectBestCamera();$('#arCamera').srcObject=cameraStream;$('#torchButton').disabled=!result.torch;$('#arMessage').textContent=result.torch?`손전등 지원 카메라 연결됨 · ${result.label}`:`후면 카메라 연결됨 · 손전등 미지원`;speakGuide(`${route.exit[0]} 방향으로 대피를 시작합니다. ${Math.round(remainingDistance)}미터 남았습니다.`,true);await enableSensors(false)}catch(error){$('#arMessage').textContent=error.name==='NotAllowedError'?'카메라 권한이 거부되었습니다. 브라우저 설정에서 허용해주세요.':'후면 카메라를 시작할 수 없습니다.'}
}
...
function onOrientation(event){const heading=event.webkitCompassHeading??(event.alpha==null?null:360-event.alpha);if(heading==null)return;updateDirection(targetBearing-heading)}
function onMotion(event){if(!cameraStream||remainingDistance<=0)return;const a=event.accelerationIncludingGravity;if(!a)return;const magnitude=Math.hypot(a.x||0,a.y||0,a.z||0),delta=Math.abs(magnitude-lastMagnitude),now=Date.now();lastMagnitude=magnitude;if(delta>1.8&&delta<7&&now-lastStepAt>380){lastStepAt=now;remainingDistance=Math.max(0,+(remainingDistance-.7).toFixed(1));$('#arDistance').textContent=remainingDistance;if(remainingDistance<=lastDistanceCallout-5&&remainingDistance>0){lastDistanceCallout=remainingDistance;speakGuide(`${Math.ceil(remainingDistance)}미터 남았습니다.`)}if(remainingDistance===0){$('#arDirection').textContent='비상구에 도착했습니다';$('#arMessage').textContent='주변 안전을 확인하고 건물 밖으로 이동하세요.';speakGuide('비상구에 도착했습니다. 주변 안전을 확인하고 건물 밖으로 이동하세요.',true)}}}
...
$('#close').onclick=$('#modalConfirm').onclick=()=>$('#modal').classList.remove('open');$('#arStart').onclick=startAr;$('#homeArStart').onclick=startAr;$('#arClose').onclick=()=>show(1);$('#torchButton').onclick=toggleTorch;$('#voiceButton').onclick=toggleVoice;$('#signalButton').onclick=toggleSignal;$('#sensorButton').onclick=()=>enableSensors(true);$('#guardian').onclick=()=>modal('보호자에게 알리기',alertSummary('보호자'));$('#manager').onclick=()=>modal('관리자에게 알리기',alertSummary('관리자'));window.addEventListener('pagehide',stopAr);$('#locationText').textContent=`${floorData[floor].name} ${currentLocation()[1]}`;renderFloor();
