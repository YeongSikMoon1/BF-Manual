...
function planRoute({startPoint=null,blockedSegment=null}={}){
...
async function shareKakaoLocation(){
 ...
 try{await initializeKakao();Kakao.Share.sendDefault({objectType:'text',text:`[BF Manual 긴급 위치 공유]\n현재 위치: ${place}\n이동 상태: ${mobility}\n추천 대피 지점: ${destination}\n현재 이 위치에 있어요. 제 위치를 확인해주세요.`,link:{mobileWebUrl:location.origin,webUrl:location.origin},buttonTitle:'BF Manual 위치 확인'});}
 catch(error){modal('카카오톡 위치 전송 준비 필요',`${error.message}\n\n배포 주소를 카카오 개발자 콘솔의 JavaScript SDK 도메인과 제품 링크 웹 도메인에 등록해주세요.\n\n${friendLocationSummary()}`)}
}
...
let obstacleModel=null,obstacleTimer=null,obstacleBusy=false,obstacleBlocked=false,lastObstacleWarningAt=0,obstacleSince=0,lastDetourAt=0,orientationReceived=false,motionReceived=false,sensorCheckTimer=null;
...
function recalculateDetour(source='신고된 통로'){if(!arRoute)return;const progress=routeProgress(arSteps,arStepIndex,remainingDistance),point=pointAlongPath(arRoute.path,progress.percent),blockedSegment=segmentAtProgress(arRoute.path,progress.percent),next=planRoute({startPoint:point,blockedSegment});if(next.noPath){$('#arMessage').textContent='현재 위치에서 확인된 우회로가 없습니다. 안전한 곳에 멈춰 도움을 요청하세요.';speakGuide('확인된 우회로가 없습니다. 안전한 곳에 멈춰 도움을 요청하세요.',true);return}arRoute=next;arDestinationName=next.exit[0];arSteps=buildArSteps(next);arStepIndex=0;arCompleted=false;renderArMap();activateArStep(0,false);$('#arMessage').textContent=`${source}를 제외하고 새 경로를 찾았습니다.`;speakGuide(`${source}를 피해 우회 경로로 안내합니다. ${arSteps[0].text}`,true)}
async function startAr(){
 const route=planRoute(),item=currentLocation();arRoute=route;arDestinationName=route.exit[0];arSteps=buildArSteps(route);arCompleted=false;currentHeading=null;lastWrongWayAt=0;$('#arLocation').textContent=`${floorData[floor].name} ${item[1]}`;show(5);renderArMap();activateArStep(0,false);setArrowMode('forward');
 if(!navigator.mediaDevices?.getUserMedia){$('#arMessage').textContent='이 브라우저에서는 카메라를 사용할 수 없습니다.';return}
 try{const result=await connectBestCamera(),video=$('#arCamera');video.srcObject=cameraStream;await video.play();$('#torchButton').disabled=!result.torch;$('#arMessage').textContent=result.torch?`손전등 지원 카메라 연결됨 · ${result.label}`:`후면 카메라 연결됨 · 손전등 미지원`;speakGuide(`${route.exit[0]}까지 안내를 시작합니다. ${arSteps[0].text}`,true);startObstacleDetection();prepareSensors()}catch(error){$('#arMessage').textContent=error.name==='NotAllowedError'?'카메라 권한이 거부되었습니다. 브라우저 설정에서 허용해주세요.':'후면 카메라를 시작할 수 없습니다.'}
}
...
async function detectObstacles(){
 ...
 try{const predictions=(await obstacleModel.detect(video,8,.55)).filter(item=>obstacleLabels.has(item.class)),width=video.videoWidth||1,height=video.videoHeight||1,blocking=predictions.filter(({bbox,class:label})=>{const [x,y,w,h]=bbox,center=x+w/2,area=w*h/(width*height);return center>width*.2&&center<width*.8&&y+h>height*.48&&area>(label==='person'?.09:.045)});drawObstacleBoxes(predictions,blocking);setObstacleState(blocking)}finally{obstacleBusy=false}
}
function setObstacleState(blocking){
 ...
 if(!obstacleBlocked){obstacleSince=0;setObstacleUi('active','AI 장애물 인식 켜짐');return}const labels=[...new Set(blocking.map(item=>item.class))].join(', ');status.textContent=`전방 대형 물체 감지 · ${labels}`;$('#arMessage').textContent='멈추세요. 전방 장애물을 확인하고 우회로를 계산합니다.';const now=Date.now();obstacleSince||=now;if(now-lastObstacleWarningAt>5000){lastObstacleWarningAt=now;speakGuide('멈추세요. 전방에 장애물이 감지되었습니다.',true)}if(now-obstacleSince>2500&&now-lastDetourAt>12000){lastDetourAt=now;obstacleSince=0;recalculateDetour('감지된 장애물')}
}
async function startObstacleDetection(){
 stopObstacleDetection();setObstacleUi('loading','AI 장애물 인식 준비 중');try{await loadObstacleModel();if(!cameraStream){setObstacleUi('off','카메라 연결 후 인식 가능');return}setObstacleUi('active','AI 장애물 인식 켜짐');obstacleTimer=setInterval(detectObstacles,850);detectObstacles()}catch(error){console.error('Obstacle detection failed',error);setObstacleUi('error','AI 장애물 인식 실패 · 눌러서 재시도');$('#arMessage').textContent='장애물 인식을 시작하지 못했습니다. 인식 버튼을 눌러 다시 시도하세요.'}
}
function stopObstacleDetection(){if(obstacleTimer){clearInterval(obstacleTimer);obstacleTimer=null}obstacleBlocked=false;obstacleBusy=false;obstacleSince=0;const canvas=$('#obstacleCanvas'),ctx=canvas?.getContext('2d');if(ctx)ctx.clearRect(0,0,canvas.width,canvas.height);$('#obstacleStatus')?.classList.remove('detected');$('.ar-screen')?.classList.remove('obstacle-blocked');setObstacleUi('off','AI 장애물 인식 꺼짐')}
...
function setSensorUi(state,text){const button=$('#sensorButton');button.classList.toggle('active',state==='active');button.classList.toggle('error',state==='error');button.querySelector('span:last-child').textContent=text}
function prepareSensors(){orientationReceived=false;motionReceived=false;if(typeof DeviceOrientationEvent?.requestPermission==='function'||typeof DeviceMotionEvent?.requestPermission==='function'){setSensorUi('required','센서 켜기');$('#arMessage').textContent='iPhone에서는 센서 켜기 버튼을 한 번 눌러주세요.'}else enableSensors(false)}
async function enableSensors(showMessage=true){try{setSensorUi('checking','센서 확인 중');if(typeof DeviceOrientationEvent?.requestPermission==='function'){const result=await DeviceOrientationEvent.requestPermission();if(result!=='granted')throw new Error('denied')}if(typeof DeviceMotionEvent?.requestPermission==='function'){const result=await DeviceMotionEvent.requestPermission();if(result!=='granted')throw new Error('denied')}if(!sensorActive){window.addEventListener('deviceorientation',onOrientation,true);window.addEventListener('devicemotion',onMotion,true);sensorActive=true}clearTimeout(sensorCheckTimer);sensorCheckTimer=setTimeout(()=>{if(!orientationReceived&&!motionReceived){setSensorUi('error','센서 미수신');$('#arMessage').textContent='센서 데이터가 들어오지 않습니다. 브라우저의 동작 및 방향 접근을 허용해주세요.'}},2500);if(showMessage)$('#arMessage').textContent='센서 연결을 확인하고 있습니다.'}catch{setSensorUi('error','센서 권한 필요');if(showMessage)$('#arMessage').textContent='브라우저 설정에서 동작 및 방향 접근을 허용해주세요.'}}
...
if(typeof document!=='undefined'){$('#close').onclick=$('#modalConfirm').onclick=()=>$('#modal').classList.remove('open');$('#arStart').onclick=startAr;$('#homeArStart').onclick=startAr;$('#kakaoLocation').onclick=shareKakaoLocation;$('#arClose').onclick=()=>show(1);$('#mapButton').onclick=()=>toggleArMap();$('#mapClose').onclick=()=>toggleArMap(false);$('#blockedRouteButton').onclick=()=>recalculateDetour('신고된 막힌 통로');$('#arMapModal').onclick=event=>{if(event.target===$('#arMapModal'))toggleArMap(false)};$('#obstacleButton').onclick=startObstacleDetection;$('#torchButton').onclick=toggleTorch;$('#voiceButton').onclick=toggleVoice;$('#signalButton').onclick=toggleSignal;$('#sensorButton').onclick=()=>enableSensors(true);$('#guardian').onclick=()=>modal('보호자에게 알리기',alertSummary('보호자'));$('#manager').onclick=()=>modal('관리자에게 알리기',alertSummary('관리자'));window.addEventListener('keydown',event=>{if(event.key==='Escape'&&!$('#arMapModal').hidden)toggleArMap(false)});window.addEventListener('pagehide',stopAr);renderFloor()}
