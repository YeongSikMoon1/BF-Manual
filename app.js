...
function setObstacleState(blocking){
 ...
 if(!obstacleBlocked){setObstacleUi('active','AI 장애물 인식 켜짐');return}const labels=[...new Set(blocking.map(item=>item.class))].join(', ');status.textContent=`전방 대형 물체 감지 · ${labels}`;$('#arMessage').textContent='멈추세요. 전방 장애물을 피해 안전을 확인하세요.';const now=Date.now();if(now-lastObstacleWarningAt>5000){lastObstacleWarningAt=now;speakGuide('멈추세요. 전방에 장애물이 감지되었습니다.',true)}
}
